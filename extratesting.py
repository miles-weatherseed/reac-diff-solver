...
def test_1d_reaction():
    ...
    D1 = 0.05
    D2 = 0.0
    t = np.linspace(0, 5.0, 51)
    k1 = 1.0
    k2 = 2.0

    def exact(x, y, t):
        return [0.1 * np.exp(-k1*t), 2.0*np.exp(-k2*t)]
    ...
    plt.figure()
    plt.plot(t, u[:, 20, 20], 'xb')
    plt.plot(t, solution[0][:,20,20], 'b')
    plt.plot(t, v[:, 20, 20], 'xr')
    plt.plot(t, solution[1][:, 20, 20], 'r')
...
def test_reactiondiffusion():
    ...
    solver = Solver([0.0, 1.0], [0.0, 1.0], 100, initial_conditions)
    solver.set_timeStepLength(0.001)
    ...
    for i in range(len(t)):
        plt.plot(solver.x, u[i, 20, :])
    #plt.plot(solver.x, solution[0][:, 20, 20], 'b')
    #plt.plot(t, v[:, 20, 20], 'xr')
    #plt.plot(t, solution[1][:, 20, 20], 'r')
...
def gray_scott():
    ...
    ax.plot_surface(solver.X, solver.Y, u[1])
